/* linux/arch/arm/mach-msm/board-vigor-mmc.c
 *
 ...
 */
static struct embedded_sdio_data vigor_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
vigor_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int vigor_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int vigor_wifi_status(struct device *dev)
{
...
int vigor_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(vigor_wifi_set_carddetect);
...
EXPORT_SYMBOL(vigor_wifi_power);
